...
for S, K in SKs:
    ...
    run_check(lib.relu_f32, torch.relu, x, "f32", y)
    run_check(lib.relu_f32x4, torch.relu, x, "f32x4", y)
    run_check(lib.relu_f16, torch.relu, x.half().contiguous(), "f16", y.half().contiguous())
    run_check(lib.relu_f16x2, torch.relu, x.half().contiguous(), "f16x2", y.half().contiguous())
    run_check(lib.relu_f16x8, torch.relu, x.half().contiguous(), "f16x8", y.half().contiguous())
    run_check(lib.relu_f16x8_pack, torch.relu, x.half().contiguous(), "f16x8pack", y.half().contiguous())
